...
<script>
  ...
  const VOTE_URL = 'admin/ajax/rqst.php';
  ...
  function renderCandidatos(list){
    ...
    candidatosBody.innerHTML = list.map((c, idx) => {
      ...
      const nombre = (c.nombre_completo ?? c.candidato ?? c.nombre ?? c.nombre_candidato ?? 'Candidato');
      const cargo = (c.cargo_publico ?? c.cargo ?? c.nombre_cargo ?? '—');
      const partido = (c.nombres_partidos ?? c.partido ?? c.partidos ?? '—');
      const municipio = (c.nombre_municipio ?? c.municipio ?? '—');
      const departamento = (c.nombre_departamento ?? c.departamento ?? '—');
      const foto = (c.foto_url ?? c.foto ?? c.imagen ?? c.url_foto ?? '');
  ...
  async function enviarVoto(){
    ...
    try{
      ...
      if (!res.ok || !data || !data?.output?.valid) {
        const msg = data?.output?.response?.content || data?.message || data?.msg || 'No fue posible registrar el voto.';
        throw new Error(msg);
      }
