...
class Pregunta
{
    ...
    public static function getAll($rqst)
    {
        ...
        $q .= " GROUP BY p.id ORDER BY p.id ASC, p.capitulo ASC, p.orden ASC";
    ...
    /**
     * Guarda el enunciado para múltiples preguntas seleccionadas
     */
    public static function saveEnunciadoSelected($rqst)
    {
        $ids = isset($rqst['ids']) ? $rqst['ids'] : '';
        $enunciado_pregunta = isset($rqst['enunciado_pregunta']) ? trim($rqst['enunciado_pregunta']) : '';

        if (empty($ids)) {
            return Util::error_missing_data_description('No se recibieron preguntas seleccionadas.');
        }
        ...
        try {
            $placeholders = implode(',', array_fill(0, count($idsArray), '?'));
            $q = "UPDATE " . $db->getTable('tbl_preguntas') . "
                  SET enunciado_pregunta = :enunciado
                  WHERE id IN ($placeholders)";
            $stmt = $pdo->prepare($q);
            $stmt->bindValue(':enunciado', $enunciado_pregunta, PDO::PARAM_STR);
            ...
            $count = $stmt->rowCount();
            $arrjson = array('output' => array('valid' => true, 'response' => "$count pregunta(s) actualizada(s) correctamente."));
        } catch (Exception $e) {
            $arrjson = Util::error_general('Error al guardar el enunciado: ' . $e->getMessage());
        } finally {
            $db->closeConect();
        }
