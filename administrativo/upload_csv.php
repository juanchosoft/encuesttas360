...
<body>
  ...
  <div class="dropzone dropzone-multiple p-0 mb-5" id="upload_button" data-dropzone="data-dropzone" style="min-height: 100px; padding: 10px;">
    ...
    <p>Arrastra tu csv aquí</p>
    <span>o haz clic para seleccionar</span>
    <img src="assets/img/icons/file.png" alt="CSV" />
  ...
  $(document).ready(function () {
    ...
    new AjaxUpload('#upload_button', {
      ...
      onSubmit: function (file, ext) {
        ...
        if (!(ext && /^(csv)$/i.test(ext))) {
          ...
          $('#uploadStatus').html("Subiendo CSV...");
          this.disable();
        }
      },
      onComplete: function (file, response) {
        $('#uploadStatus').html("<span style='color: green;'>CSV Cargado correctamente</span>");
        $('#valor_iframe').val('1');
        this.enable();
    ...
    dropZone.addEventListener("drop", function (e) {
      ...
      if (files.length > 0) {
        ...
        const extensiones_permitidas = ["csv"];
        ...
        .catch(err => {
          console.error(err);
          $('#uploadStatus').html("<span style='color: red;'>Error al subir csv</span>");
        });
      }
    });
  });
