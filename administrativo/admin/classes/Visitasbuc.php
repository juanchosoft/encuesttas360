...
class Visitasbuc
{
  ...
  public static function getAll($rqst)
  {
    ...
    $q="SELECT tbl_visitas_com.id, tbl_visitas_com.entidad, tbl_visitas_com.barrio, tbl_ciudades.comuna, tbl_visitas_com.beneficiario, tbl_visitas_com.compromisos, tbl_visitas_com.img, tbl_visitas_com.date, tbl_visitas_com.observaciones, tbl_visitas_com.compromisos
    FROM " . $db->getTable('tbl_visitas_com') . "
    INNER JOIN " . $db->getTable('tbl_ciudades') . " ON tbl_visitas_com.tbl_municipio_id = tbl_ciudades.codigo_muncipio 
    ORDER BY tbl_visitas_com.date";
 

    if ($id > 0) {
      $q = "SELECT * FROM " . $db->getTable('tbl_visitas_com') . " WHERE id = " . $id;
    }

    if ($tipo != "") {
      $q = "SELECT * FROM " . $db->getTable('tbl_visitas_com') . " ";
    }
    if ($tbl_municipio_id != "") {
      $q = "SELECT * FROM " . $db->getTable('tbl_visitas_com') . " WHERE tbl_municipio_id=".$tbl_municipio_id;
    }
  ...
  public static function save($rqst)
  {
    ...
    if ($id > 0) {
      $q0 = "SELECT * FROM " . $db->getTable('tbl_visitas_com') . " WHERE id = " . $id;
      $result0 = $pdo->query($q0);
      if ($result0) {
        $table = $db->getTable('tbl_visitas_com');
      ...
      if ($date != "" || $entidad != "" || $tbl_departamento_id > 0 || $beneficiario != "") {
        $q = "INSERT INTO " . $db->getTable('tbl_visitas_com') . " (created_at, date, entidad, cargo, beneficiario, provincia, observaciones, compromisos, img, tbl_departamento_id, tbl_municipio_id,  tbl_usuario_id) VALUES
