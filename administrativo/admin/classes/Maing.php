...
class Maing
{
  public function __construct() {}
    ...
    public static function getDataMain($rqst)
    {
      ...
      $q1 = "SELECT SUM(poblacion) AS impactada FROM " . $db->getTable('tbl_gestora');
           $impactada = $pdo->query($q1)->fetchColumn();

      // Consulta 2: Total de visitas realizadas
      $q2 = "SELECT COUNT(id) AS cuenta_visitas FROM " . $db->getTable('tbl_gestora');
      $visitas = $pdo->query($q2)->fetchColumn();
      $compromiso = $visitas; // Compromisos y visitas parecen ser iguales
      ...
      $q6 = "SELECT SUM(inversion) AS inversion FROM " . $db->getTable('tbl_gestora');
