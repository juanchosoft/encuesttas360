...
class Permiso
{
    ...
    public static function permisos($rqst)
    {
        ...
        $q = "SELECT * FROM " . $db->getTable('tbl_usuarios_has_tbl_permisos') . " WHERE tbl_usuarios_id ='" . $id . "' ORDER BY tbl_permiso_id ASC";
        $result = $pdo->query($q);
        $arrassigned = array();
        ...
        if ($result) {
            ...
            $q1 = "SELECT * FROM " . $db->getTable('tbl_permisos') . " ORDER BY nombre ASC";
    ...
    public static function savePermisos($rqst)
    {
        ...
        if ($id > 0) {
            ...
            for ($i = 0; $i < count($arrchk); $i++) {
                ...
                if ($prf_id > 0) {
                    $q1 = "INSERT INTO " . $db->getTable('tbl_usuarios_has_tbl_permisos') . " (dtcreate,tbl_usuarios_id,tbl_permiso_id)  VALUES ( " . Util::date_now_server() . ", $id, $prf_id)";
                    $result1 = $pdo->query($q1);
                    if ($result1) {
