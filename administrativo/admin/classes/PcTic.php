...
class PcTic
{
  ...
  public static function save($rqst)
  {
    ...
    $tbl_vereda_id = isset($rqst['tbl_vereda_id']) ? trim($rqst['tbl_vereda_id']) : '';;
    $nombre_vereda = isset($rqst['nombre_vereda']) ? trim($rqst['nombre_vereda']) : null;
    ...
    if ($date == "") {
      $arrjson = Util::error_general('Fecha es obligatorio');
      return $arrjson;
    }
    if ($tbl_departamento_id == "") {
    ...
    if ($id > 0) {
      ...
      if ($date != "" || $tbl_departamento_id > 0 || $zona != "") {
