...
const EstudioVotaciones = {
  ...
  init: function(grillaData, preguntasConfig, subpreguntasConfig) {
    ...
    console.log('Inicializando Sistema de Estudio de Votaciones - DINÁMICO');
    console.log('Datos de la grilla:', this.grillaData);
    console.log('Preguntas configuradas:', this.preguntasConfig);
    console.log('Subpreguntas configuradas:', this.subpreguntasConfig);
      ...
      if (response && response.output && response.output.valid) {
        ...
        if (yaVoto) {
          ...
          if (btnGuardar) {
            ...
            btn Guardar.style.cursor = 'not-allowed';
  ...
  initializeRespuestas: function() {
    ...
    rows.forEach(row => {
      ...
      this.preguntasConfig.forEach((pregunta, index) => {
        ...
        const opciones = JSON.parse(pregunta.opciones_respuesta || '[]');
    ...
    console.log('Subpreguntas inicializadas:', this.subpreguntasRespuestas);
  },
  ...
  handleToggleClick: function(candidatoId, codigoPregunta, valor, row, btn, td) {
    console.log(`Click en candidato ${candidatoId}, pregunta: ${codigoPregunta}, valor: ${valor}`);
  ...
  applyConditionalLogicDynamic: function(candidatoId, codigoPregunta, valor, row, tdActual) {
    ...
    console.log(`Pregunta: ${codigoPregunta}, Habilita: ${habilita}, Condición: ${condicion}`);
  ...
  todasRespuestasPositivas: function(candidatoId) {
    ...
    for (const pregunta of this.preguntasConfig) {
      ...
      if (!esPositiva || respuesta === 'no_aplica') {
        return false;
      }
    }
  ...
  /**
   * Habilita la siguiente pregunta
   * @param {HTMLElement} row - Fila de la tabla
  ...
  habilitarSiguientePregunta: function(row, ordenActual) {
    ...
    const tdSiguiente = row.querySelector(`td[data-orden="${ordenSiguiente}"]`);

    if (tdSiguiente) {
      const preguntaSiguiente = this.preguntasConfig.find(p => parseInt(p.orden) === ordenSiguiente);
      if (preguntaSiguiente) {
        const opciones = JSON.parse(preguntaSiguiente.opciones_respuesta || '[]');
        const valorDefault = opciones[0] || 'si'; // Primera opción por defecto

        this.setAplicable(tdSiguiente, valorDefault);

        // Actualizar respuesta
        const candidatoId = row.getAttribute('data-candidato-id');
        this.respuestas[candidatoId][preguntaSiguiente.codigo_pregunta] = valorDefault;
      }
    }
  },
  ...
  applyInitialState: function() {
    console.log('Estado inicial aplicado - Primera pregunta activa, resto deshabilitado');
    this.actualizarCandidatosAprobados();
  },
  ...
  getCandidatosAprobados: function() {
    ...
    for (const candidatoId in this.respuestas) {
      if (this.todasRespuestasPositivas(candidatoId)) {
  ...
  renderizarSubpreguntasDinamicas: function(aprobados, letras) {
    ...
    preguntasSection.innerHTML = '<h6 class="text-center mb-3"><i class="fas fa-poll"></i> PREGUNTAS ADICIONALES</h6>';
   ...
   * @returns {HTMLElement}
   */
  crearSubpreguntaDinamica: function(subpregunta, aprobados, letras, index) {
    ...
    setTimeout(() => {
      ...
      botones.forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          console.log(`Subpregunta ${codigoSubpregunta}: ${candidatoId}`, self.subpreguntasRespuestas);
        });
      });
    }, 100);
  ...
  guardarRespuestas: function() {
    ...
    console.log('Subpreguntas:', this.subpreguntasRespuestas);

    // Validar respuestas completas
    const aprobados = this.getCandidatosAprobados();
    if (aprobados.length > 0) {
      // Validar subpreguntas
      for (const subpregunta of this.subpreguntasConfig) {
        if (this.subpreguntasRespuestas[subpregunta.codigo_pregunta] === null) {
          UTIL.mostrarMensajeValidacion(
            'Por favor, complete todas las subpreguntas:<br><br>' +
