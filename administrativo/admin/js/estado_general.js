var ESTADO_GENERAL = {
    ...
    createChartComparativa: function (container, title, jsonData, tipo) {
        ...
        // Limpiar el contenido previo del container
        const containerElement = document.querySelector(`#${container}`);
        if (containerElement) {
            containerElement.innerHTML = "";
        }
        // Renderizar el gráfico
        const chart = new ApexCharts(containerElement, options);
        chart.render();
    },
    ...
    getTotalVisitasPorProvincia: function () {
        this.fetchDataComparativa("gettotalvisitasporprovincia", "containerProvincias", "Visitas realizadas a Provincias")
            .then((data) => {
    ...
    getTotalVisitasPorMesDelDepartamento: function () {
        this.fetchDataComparativa("gettotalvisitaspormesmunicipio", "containerMunicipios", "Visitas por Mes a Municipios")
            .then((data) => {
...
//     showData: function(color) {
//         q = {};
//         q.op = "getveredasbycolor_depart";
//         q.color = color;
//         if ($('#municipioSelect').val() === 'antioquia') {
//             q.codigo_departamento = 5;
//         }
//         q.color = color;
//         if ($('#municipioSelect').val() === 'choco') {
//             q.codigo_departamento = 27;
//         }
//         UTIL.callAjaxRqstPOST(q, this.showDataHandler);
//     },
//     showData2021: function(color) {
//         q = {};
//         q.op = "getveredasbycolor_depart2021";
//         q.color = color;
//         if ($('#municipioSelect').val() === 'antioquia') {
//             q.codigo_departamento = 5;
//         }
...
//         UTIL.callAjaxRqstPOST(q, this.showDataHandler);
//     },
//     showDataMunicipio: function(color) {
//         q = {};
//         q.op = "getveredasbycolor_munic";
//         q.color = color;
//         q.codigo_municipio = $('#tbl_municipio_id').val();
//         UTIL.callAjaxRqstPOST(q, this.showDataHandler);
//     },
//     showDataMunicipio2021: function(color) {
//         q = {};
//         q.op = "getveredasbycolor_munic2021";
//         q.color = color;
//         q.codigo_municipio = $('#tbl_municipio_id').val();
//         UTIL.callAjaxRqstPOST(q, this.showDataHandler);
//     },
...
//     },
// };
function actualizarMapaIframe() {
    const dep = document.getElementById('tbl_departamento_id').value;
    const mun = document.getElementById('tbl_municipio_id').value;

    if (!dep || !mun) return;

    // AJAX para obtener el nombre real del municipio (nombre_mapa)
    fetch(`admin/ajax/get_nombre_municipio.php?mun=${mun}`)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                const nombre = data.nombre.toLowerCase();
                const iframe = document.getElementById('iframeMapa');
                iframe.src = `admin/mapa-veredas/${dep}/${nombre}/${nombre}.php`;
            } else {
                console.warn("No se encontró el municipio en la BD.");
            }
        });
}
