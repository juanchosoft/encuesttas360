...
const EstudioVotaciones = {
  ...
  habilitarSiguientePregunta: function(row, ordenActual) {
    ...
    console.log(`Habilitadas todas las preguntas posteriores a orden ${ordenActual}`);
  },
  ...
  getCandidatosAprobados: function() {
    ...
    console.log(`Pregunta activadora de subpreguntas: ${codigoPreguntaActivadora}`);
    ...
    console.log('Candidatos aprobados para subpreguntas:', aprobados);
    return aprobados;
  },
  ...
  crearSubpreguntaDinamica: function(subpregunta, aprobados, letras, index, soloLectura = false) {
    ...
    if (soloLectura && aprobados.length === 1) {
      ...
      console.log(`Subpregunta ${codigo}: Auto-asignada al único candidato ${aprobados[0]} (solo lectura)`);
    }
  ...
  actualizarOpcionesSubpreguntas: function(subpreguntaIndex) {
    ...
    console.log('Candidatos ya seleccionados:', candidatosSeleccionados);
  ...
  guardarRespuestas: function() {
    ...
    const votanteSelector = document.getElementById('votanteSelector');
    const votanteId = votanteSelector ? votanteSelector.value : '';

    if (!votanteId || votanteId === '') {
      UTIL.mostrarMensajeValidacion('Por favor, seleccione un votante antes de guardar las respuestas.');
      return;
    }
