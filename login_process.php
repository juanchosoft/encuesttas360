...
try {
  ...
  if ($isvalid) {
    ...
    // Redirigir según opción activa
    $opcionActiva = $user['opcion_activa_web'] ?? 'sondeo';
    $redirect = 'dash_responder.php';
    if ($opcionActiva === 'sondeo' || $opcionActiva === 'cuestionario' || $opcionActiva === 'ambos') {
      $redirect = 'dash_responder.php';
    }

    jsonExit(['status' => 'success', 'redirect' => $redirect]);
  }
